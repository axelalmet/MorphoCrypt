...
sol = ode45(InnerDerivFun, [0, 0.5], initInnerPoints, solOptions);

% Evaluate the solution at the contact point
w0 = modelParams.w0; % Contact width

innerSol = deval(sol, sC);
innerResiduals = innerSol([1; 5]) - [w0; -0.5*pi];

outerSol = deval(sol, 0.5);
outerResiduals = outerSol([1; 2; 5]) - [0.5; 0; 0];
